...
from complexity.utils.file_utils import (
    load_aql_query, project_root, load_env_file
)
from complexity.rag.database.arango_utils import (
    create_arangosearch_view, store_docs_in_arango,initialize_database
)
...
def retrieve_top_docs(question_text: str, db, embedder: ModernBertEmbedder, config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # 2) AQL
    aql = load_aql_query(PROJECT_ROOT, "rag_classifier.aql")
