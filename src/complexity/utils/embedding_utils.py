...
def cosine_similarity(vec1: List[float], vec2: List[float]) -> float:
    ...
    if len(vec1) != len(vec2):
        raise ValueError(
            f"Vectors must have the same length: {len(vec1)} != {len(vec2)}"
        )
...
def validate_embeddings(
    test_embeddings: List[Dict[str, Any]], fixture_path: str
    ...
    try:
        ...
        return False, {
            "fixture_error": {
                "expected": "Fixture loaded successfully",
                "actual": str(e),
            }
        }
    ...
    # Check dimensions
    expected_dimensions = expected_data.get(
    ...
    if OPENAI_AVAILABLE:
        filtered_failures = {
            k: v for k, v in validation_failures.items() if "dimensions" in k
        }
...
if __name__ == "__main__":
    ...
    try:
        ...
        for text in test_texts:
            ...
            logger.info(
                f"Generated embedding for '{text[:20]}...' with {len(embedding)} dimensions"
            )
        ...
        for i in range(len(test_embeddings)):
            for j in range(i + 1, len(test_embeddings)):
                ...
                logger.info(
                    f"Similarity between '{text1[:10]}...' and '{text2[:10]}...': {similarity:.4f}"
                )

        # Validate the embeddings
        validation_passed, validation_failures = validate_embeddings(
            test_embeddings, fixture_path
        )

        # Report validation status - Correctly check boolean
        if validation_passed:
            print("✅ VALIDATION COMPLETE - All embedding functions work as expected")
            sys.exit(0)
        else:
            print(
                "❌ VALIDATION FAILED - Embedding functions don't match expected behavior"
            )
            ...
            for field, details in validation_failures.items():
                ...
                print(
                    f"  - {field}: Expected: {details['expected']}, Got: {actual_val}"
                )
